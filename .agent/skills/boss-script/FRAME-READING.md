...
- **브라우저 창이 가려져 있으면**(pane hidden) `requestAnimationFrame`이 돌지 않아 `ccShot`이 멈춘다 — 그 기다림을
  `setTimeout`(0.9초쯤)으로 바꾼다. 스크린샷도 **한 장씩 밀린다**(직전 그림이 찍힌다) — 작은 스크린샷(`scale` 0.1)을
  한 장 버리고 찍는다. 캔버스에 찍어 둔 영상 시각(`v=`)으로 밀렸는지 늘 확인한다.
...
## 6. 좌표 모드 — 화면 좌표 재기 (S40에서 처음 썼다 — `docs/bosses/사치스러운 거미.md` §좌표를 잰 방법)
...
1. **환산 비율** — 화면의 조준 원(레티클)은 그 니케 무기의 탄착군이다. CDN 직경을 아는 무기(AR 75 · SMG 110 ·
   ...
   가로 1080p PC 영상에서는 **k = 1920/1080 ≈ 1.78**이 맞았다(S40: SMG 레티클 62px ↔ 110 · SG 탄착원 135px ↔ 240) —
   CDN px가 세로 1080×1920 기준 좌표로 보인다. 먼저 이 값으로 대 보고 어긋나면 잰다. **SG 레티클의 네 조각 십자선은
   탄착군이 아니다** — 그 뒤의 반투명 원이 탄착군이다. 레티클 옆 숫자는 남은 장탄이라(장탄 증가가 붙는다) 무기 판별에
   쓰지 않는다.
2. **기준점** — 오토 니케가 쏘는 자리(자동 에임 위치, 대개 코어)를 원점으로 둔다. 코어가 있으면 코어 중심이다.
   그러면 스크립트의 `coord.auto_aim`은 `[0, 0]`이다. 코어가 없으면 **유저에게 묻는다**(SKILL §0 — S40은 흉부).
...
4. **카메라 확대에 주의한다** — SR·RL은 차지 중 화면이 확대된다(CDN `zoom_rate`). 확대된 프레임에서 잰 값은 비율이
   ...
   카메라 니케를 바꾸면 화면이 옆으로 움직이고 보스도 몸을 들썩인다 — 표적은 **같은 프레임의 보스 몸통**을 기준으로 재서
   기준점으로 옮긴다. 보스가 다가오는 연출(S40 암전)은 배율이 달라 기준점을 그 화면에서 새로 찾는다.
...
7. **세트로 차례로 나오는 저지원** — 세트마다 자리·크기와 출현 지연(WARNING → 첫 세트 · 세트 사이)을 적는다. 스크립트는
   SKILL §3의 타이머 + 사슬로 적는다. 저지원에는 안쪽 흰 구와 겉 점선 고리가 있다 — S40에서는 안쪽 구를 판정으로 봤다(⬜).
   공략 글의 배치 그림(순서·세트 묶음)을 먼저 보면 프레임에서 무엇을 찾을지가 빨리 잡힌다.
