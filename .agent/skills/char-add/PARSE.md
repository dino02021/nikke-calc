...
7. **원문 대조를 돌린다**: `python -m runner.parsecheck "<이름>"`. clause별로 원문 블록 → 받은 효과를
   늘어놓고 코드가 판정한다. 다 끝났다는 AI의 확신을 대조표가 대신한다.
   - **N·O·P·R·S에 ✗가 있으면 고친다** — 값(`{i}` 레벨 1~10 값 열)·부속 블록(`[N초 유지]` 등)·화살표
     부호·키 숫자·문구 조각은 해석의 여지가 없는 자리다. 원문 밖 지식에서 온 결정만
     `runner/parsecheck.py`의 `EXEMPT`에 사유와 함께 올린다(`doclint`가 같은 판정을 게이트로 돈다).
