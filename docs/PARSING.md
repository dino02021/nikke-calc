...
> **상태의 담체 — 상태 이름은 반드시 `지속 효과`에 붙인다.**
> `self_state:X` · `not_self_state:X` · `event:state_end:X`는 **X라는 이름의 버프가
> 살아 있는가**로 판정된다(`buff_manager._has_self_state`, `tick()`의 만료 처리).
> `instant`는 그 목록에 들어가지 않으므로 — `[N초 간격]` 주기 instant도 별도 타이머로
> 돈다 — 번호 없는 이름이 instant에만 붙으면 **그 상태는 영원히 성립하지 않고 종료
> 이벤트도 안 뜬다.** 조건이 거짓일 뿐이라 에러도 로그도 없이 조용히 꺼진다.
> 그러니 한 clause가 `[상태명]`으로 묶일 때, 번호 없는 이름은 **그 상태와 같이 사는
> 지속 효과**에 주고 instant는 뒤 번호로 민다.
> 예 — 그레이브 스킬3 `■ 자신에게 [미래 예지] [현재 체력 1% ▼]…[장탄 수 무한] [10초 유지]`:
> `미래 예지` = 장탄 수 무한(10초 buff), `미래 예지 5` = 현재 체력 ▼(주기 instant).
> 반대로 붙였던 동안 뒤따르는 `방열` 계열 5개와 `과열 II·III`가 통째로 미발동이었다
> (2026-08-28 수정, 그레이브 개인 딜 +24%). `runner/doclint.py` 검사 K가 강제한다.
>
...
- **캐릭터 전체 파싱 결과에서 `name` 중복 금지.** 같은 이름 생기면 첫 번째는 원래 이름 유지, 두 번째부터 ` 2`, ` 3` suffix (예: `"미사일"`, `"미사일 2"`, `"미사일 3"`). calculator가 `target_effect` 등으로 참조 시 첫 번째 항목 기준.
  ...
  >
  > **엔진은 이런 항목을 한 상태로 다룬다** — 같은 stat을 같은 대상에게 거는 두 항목은 겹쳐 떠도 병존하지 않고
  > 뒤의 것이 앞의 것을 갱신한다(`GAMEPLAY.md` §버프 스택, 2026-09-24). 인스턴스는 먼저 건 항목의 값을 쓰므로
  > **항목끼리 `values`·`fixed_value`가 같아야 한다** — `runner/doclint.py` 검사 M이 강제한다.
  >
...
| `버스트 N단계 진입 시` / `버스트 N단계 돌입 시` | `"burst_enter:N"` |
| `버스트 N단계 돌입 시` + `[시작 횟수 별 효과]` (단계 누적형) | `"burst_enter_count:N:M"` — 스쿼드가 N단계에 **M번째 이상** 들어설 때마다(`burst_cast_count:M`과 같은 `>= M` 규약). `[시작 횟수 별 효과]`는 선례가 트리거마다 그 트리거의 횟수판으로 적는다(풀 버스트 시작 → `full_burst_start_count` · 버스트 사용 → `burst_cast_count`). `full_burst_start_count:M`으로 접지 않는다 — 3단계 돌입은 B3 버스트 **전**이라 켜지는 시각이 다르다 (네온 : 블루 오션 `워터 제트`) |
...
| `파츠 N회 명중 시` / `대상 파츠 N회 명중 시` | `"part_hit_count:N"` — 뒤 표기는 「대상 코어 명중 시」(`core_hit_count:1`, 마리 `일점 저격 2`)와 같은 흔들림이다 (루마니 `머슬 타임`) |
...
| `전투불능에 이르는 공격에 피격 시` | `"event:lethal_hit"` — 쓰러진 **뒤**(`event:self_down`)가 아니라 쓰러질 발을 **받기 전**이다. 같은 clause의 불굴이 그 발을 받는다 (마키마 `발각된 모양이네`) |
...
| `N명 이상 동시 명중 시` / `N기 이상을 동시에 명중시켰다면` | `"multi_hit:N"` — 뒤 표기는 앞 clause(풀 차지 공격 시)에 이어지는 조건형 어미지만 판정 자체가 사건이라 timing으로 적는다 (니힐리스타 `버닝 샷 3`) |
...
| `타겟이 출현 시` / `타겟 출현 시` / `타겟이 출현했다면` | `"event:enemy_spawn"` + `max_trigger: 1` — **`event:target_spawn`을 쓰지 않는다**(유저 결정 2026-09-20). 그 이벤트는 기본 경로에 호출처가 없어(보스 스크립트 `emit` 전용, `IMPL-STATUS.md`) 영구 무발동이 된다. `GAMEPLAY.md` §condition의 「`타겟 등장 시`는 `랩쳐 등장 시`와 같이 `battle_start` 1회」(유저 결정 2026-09-19)를 따른다 (일레그 `패스트 차지 2`) |
...
**「공격 시」와 「명중 시」는 다른 축이다.** 원문이 두 단어를 구분해 쓰므로 파싱도 구분한다 —
같은 스킬 안에 둘이 나란히 오는 경우가 실제로 있다(레이 `선두 제압` — 「100회 **공격** 시」 자기
버프 + 「100회 **명중** 시」 딜).

- **발사(`on_attack` 계열)** — 탄 1발을 쏘면 1회. 총구·펠릿 수와 무관하다.
  **빗나간 탄도 발동시킨다**(유저 확인, 2026-09-04).
- **명중(`hit_count` 계열)** — **탄 단위**라 총구 수만큼 발동한다. 총구 2개는 탄 1발 소모에
  공격 1회·명중 2회다. 펠릿은 한 탄을 나눈 것이라 곱하지 않는다(`펠릿 N회 명중 시`가 따로 있다).
  빗나간 탄은 발동시키지 않는다.
- **「공격 명중 시」는 명중으로 읽는다**(유저 판단, 2026-09-04. 아스카 : WILLE `안티 AT 필드`).
  반대로 「N발 당」은 탄 소모 기준이라 발사다(같은 스킬 `섬멸 태세 추가 효과`).
- 계산기에는 아직 **빗나감 모델이 없다** — 탄착군은 코어히트 확률로만 환산되고 모든 탄이
  명중한다. 그래서 두 계열의 발동 횟수는 총구 2개인 캐릭터를 빼면 지금은 같다.
  분리는 미스 모델이 들어올 자리를 미리 갈라 둔 것이다.
...
| `자신이 불굴 상태라면` | `"self_undying"` — 위 `self_stun_immune`과 같은 규약. 불굴은 버프 이름이 아니라 상태(stat `undying`)라 `self_state:`를 쓰지 않는다(남이 건 불굴도 참) (마키마 `조용히 해주겠니? 3`) |
| `대상이 [상태명] 상태라면` | `"target_state:상태명"` |
| `대상이 [코드] 코드라면` | `"target_code:[코드]"` (예: `"target_code:전격"`) |
| `대상이 타겟이라면` (명중·공격 트리거 뒤 — 그 한 발의 대상) | `"target_is_boss"` — 시전자가 겨눈 적이 보스(타겟)인가. 쫄몹이 없으면 항상 참. `대상이 [코드] 코드 타겟이라면`은 `target_code:`와 함께 적는다(로산나 `벤데타 2` 애장품 2) |
| `대상이 타겟이 아닌 랩쳐라면` | `"target_is_add"` — 위의 부정(겨눈 적이 쫄몹). `대상이 [상태명] 상태가 아닌 랩쳐라면`(`not_target_state:`)과 다른 문구다(리버렐리오 `완만류`) |
...
| `엄폐물이 파괴된 아군이 없다면` | `"no_broken_cover_ally"` — 시전자 포함 산 아군 중 엄폐물이 부서진 아군이 없다. 앞 clause의 대상 `엄폐물이 파괴된 아군 무작위 N기`(`allies_broken_cover_random:N`)와 배타 분기라, **같은 트리거의 부활(`cover_revive`)을 배열 맨 뒤에 둔다** — 게임은 시전 순간에 분기와 대상을 한 번에 정하는데 계산기는 효과마다 판정하므로, 부활이 먼저 돌면 되살아난 엄폐물 때문에 이 조건이 참으로 뒤집혀 두 분기가 다 켜지고 부활 대상에게 같이 가는 효과도 후보를 잃는다. 기본 경로에서는 늘 참이다 (릴리 `최고의 엔지니어! 3`) |
| `재장전 중` | `"during_reload"` |
| `포커싱 상태` | `"focusing"` |
...
| `대상이 [스택명] 최대 중첩 상태라면` / `대상이 [스택명] N 중첩 이상이라면` | `"target_stack_above:스택명:N"` — 위의 **대상(적)판**. `target_state:`는 존재만 보므로 중첩 수가 조건이면 이쪽이다 (프림 `일어남` 애장품 1단계) |
...
| `남은 체력 비율이 가장 낮은 적 N기에게` | `"enemies_lowest_hp:N"` — 수치판과 같은 키다. 아군판(`allies_lowest_hp:N`)이 수치·비율을 한 정렬로 읽는 규약을 적판에도 그대로 쓴다 (네베 `북극곰의 힘`). 단일 보스에서는 갈리지 않고, 쫄몹이 있는 보스 패턴에서는 수치·비율 정렬이 다를 수 있다(⬜ 미구분) |
...
| `대상에게` | `"target"` |
| `지정한 적 대상 N기에게` | `"target"` — 조준 대상. 뒤따르는 「대상이 타겟이라면 동일 적 대상에게」는 조준 대상 기준이라 condition `target_is_boss`다 (바이퍼 `스네이크 바이트`) |
| `타겟에게` | `"boss"` — 타겟(보스) 1기. 쫄몹이 있어도 보스다(2026-09-19 — 전에는 `target`. 쫄몹이 없으면 같다). 디젤 : 윈터 스위츠 `노래할게요! 3` |
| `대상 본체에게` | `"target_body"` |
| `(적 전체 공격 뒤) 대상이 타겟이라면 동일 적 대상에게` | `"boss"` — 맞은 적 중 타겟(보스)에게만. 조건이 아니라 대상이다 — 적 전체를 때린 뒤라 보스는 늘 맞았고, 겨눈 적과 무관하다(디젤 : 윈터 스위츠 `라라라♬ 3` · 나유타 `위선 6` · 팬텀 `비기 괴도 난무 2`). 앞 대미지가 조준 대상이면 `target_is_boss` 조건 쪽이다 |
...
| `reload_speed_pct` | 재장전 속도 % ▲ — 재장전 **1회에 걸리는 시간** |
| `reload_ratio_pct` | 재장전 비율 % ▲/▼ — 재장전 **1회가 채우는 탄창 비율**에 곱해진다. 속도와 다른 축이다: 50% ▼면 시간은 그대로고 채우는 양이 절반이라 재장전 횟수가 두 배가 된다 (그레이브 `방열`) |
...
| `reload_time` | 생략 |
| `core_dmg_mult` | 생략 |
| `fire_rate` | 생략 (CDN 연사 → 무기군 기본 연사) — CDN 값이 없고 원문에 `공격 속도 N% ▼`가 있으면 환산한 절대값을 적는다 |
...
**묻기 전에 선례를 본다.** `python -m runner.precedent "<clause 원문>"` — 파싱이 끝난 캐릭터의
원문이 전부 색인돼 있어, 아래 「불명확」 계열은 대부분 같은 문구를 앞서 어떻게 파싱했는지가
답한다. 선례를 따랐으면 근거(캐릭터 이름)를 남기고 묻지 않는다. `⚠ 선례 불일치`가 뜨면
스스로 고르지 않는다 — 이미 둘로 갈린 자리라 그때는 질문이다.

### 10-A. 선례가 답하면 해소된다

- **10-1 trigger 불명확**: 대괄호 앞 텍스트가 알려진 패턴에 맞지 않음
- **10-2 target 불명확**: 대상 텍스트가 알려진 패턴에 맞지 않음
- **10-6 timing 불명**: template에 알려진 timing 키워드 없고 쿨타임 필드도 없음
- **10-8 polarity 판단 불명확**: 이로운/해로운 어느 쪽인지 결정 불가

넷 다 **문구 → 키 매핑**이라 같은 문구의 선례가 그대로 답이 된다. 선례가 없으면 그때는
그대로 질문 대상이다.

### 10-B. 선례와 무관하게 멈추고 묻는다

- **10-0 알 수 없는 블록**: Step 4 분류표와 4~6절 어디에도 매핑 안 되는 블록 → **스킵하지 않는다.** Step 4 아래 ①②③(기존 키 재활용 → 새 키를 IMPL-STATUS에 ❌로 등록하고 매핑 행 추가 → 그래도 안 되면 질문) 순서로 처리하고, 늘린 규칙은 그 자리에서 문서에 반영한다. clause 내 파싱 항목이 하나도 없을 때만 clause 전체 `_unparseable` 마킹 후 즉시 질문. 패턴 추론·유추 금지 — 규칙을 늘리는 것과 없는 규칙을 지어내는 것은 다르다.
- **10-3 스택 단계별 효과**: `단계 별 효과만 적용` 등 각 단계 수치가 다를 때
- **10-4 복잡 메카닉**: 위 규칙으로 표현 불가한 고유 메카닉
- **10-5 값 불일치**: template `{N}` 개수와 values 배열 길이 불일치
- **10-7 weapon_type 미명시**: 무기변경 스킬인데 변경 무기 유형이 스킬 설명에 없음

이 다섯은 **이 캐릭터의 데이터가 규칙 밖**이라는 뜻이라 선례 조회로 풀리지 않는다. 남이
어떻게 파싱했는지를 안다고 표현 불가한 메카닉이 표현 가능해지지 않고, 없는 정보가 생기지도
않는다.

### 질문의 시점

그 자리에서 물을지 모아서 물을지는 skill이 정한다. `char-add` 기본 모드는 10-B와
`⚠ 선례 불일치`만 즉시 묻고 나머지는 단계 3 보고로 모은다
(`.agent/skills/char-add/SKILL.md` §Gate). 단계별 모드는 나오는 자리에서 전부 묻는다.
