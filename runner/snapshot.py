...
SQUADS: dict[str, dict] = {
    ...
    "S39_레이븐레드후드": {
        ...
        # 거의 무효다 — 급소 공략의 짝(`템페스트` `hits_parts`)은 풀버스트 직전에 맞아 전투 시작분이
        # 첫 버스트 한 번만 덮고, 나머지 둘은 조건이 `not_self_state:A.N. 모드`인데 레이븐이
        # A.N. 모드로 사는 캐릭터다. `has_parts` + `part_break_interval` 45초를 켜면 `일점 공격`은
        # 0회이고 총딜은 첫 `템페스트` +566,010(+0.01%)만 움직인다(기대값 모드, 2026-09-24).
