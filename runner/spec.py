...
def build_config(squad: list[dict], config: dict | None = None) -> dict:
    ...
    **종전 표기 `char["burst_pattern"]`도 받고, 있으면 그쪽이 이긴다.** 보고서 스펙·CLI
    ...
    # 레이어 2 「저지 우선 타격」 — 패턴 모드 보스에서 카메라를 가진 니케가 산 저지원 → 쫄몹 → 안 깨면 벌칙 분기가 오는
    # 파츠 순으로 겨눈다(유저 결정 2026-09-18 · 09-19, `boss_pattern` §조준). 엔진 기본은 레이어 1(오토는 저지 때도 에임을
    # 안 옮긴다)이라 러너가 켠다. 호출자가 값을 주면(`--auto` 전원 = 레이어 1) 그대로 둔다. 패턴 모드가 아니면 엔진이 읽지 않는다
